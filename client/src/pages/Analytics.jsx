...
export default function Analytics({ applications, loading }) {
  ...
  return <div>
    ...
    {loading ? <div className="mt-6 grid gap-4 xl:grid-cols-[1.5fr_1fr]"><div className="h-96 animate-pulse rounded-2xl glass-soft" /><div className="h-96 animate-pulse rounded-2xl glass-soft" /></div> : <div className="mt-6 grid gap-4 xl:grid-cols-[1.5fr_1fr]">
      ...
      <Card className="min-w-0 p-5 sm:p-7"><h2 className="text-lg font-semibold">Status breakdown</h2><p className="mt-1 text-sm text-subtle">Where opportunities stand today</p><div className="relative mt-2 h-[215px]"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={breakdown.filter((item) => item.value > 0)} dataKey="value" cx="50%" cy="50%" innerRadius={66} outerRadius={88} paddingAngle={4} stroke="none">{breakdown.filter((item) => item.value > 0).map((item) => <Cell key={item.name} fill={item.color} />)}</Pie><Tooltip contentStyle={tooltipStyle} labelStyle={{ color: "var(--text)" }} itemStyle={{ color: "var(--text)" }} /></PieChart></ResponsiveContainer><div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center"><span className="text-3xl font-bold">{applications.length}</span><span className="text-xs text-subtle">total</span></div></div><div className="grid grid-cols-2 gap-x-5 gap-y-3">{breakdown.map((item) => <div key={item.name} className="flex items-center justify-between gap-3 text-xs"><span className="flex items-center gap-2 text-muted"><span className="size-2 rounded-full" style={{ backgroundColor: item.color }} />{item.name}</span><span className="font-semibold tabular-nums">{item.value}</span></div>)}</div></Card>
    </div>}
